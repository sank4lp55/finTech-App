...
const { checkSuperAdminByUID } = require("../services/user.service");
const auth = getAuth();
...
const validateUser = async (req, res, next) => {
  ...
  auth.verifyIdToken(token).then((decodedToken) => {
    ...
    console.log("Super Admin Auth Middleware Calling--->");
    // to verify the user
    auth
      ...
      .then(async (userRecord) => {
        ...
        try {
          ...
          const uData = await checkSuperAdminByUID(userRecord.uid);

          // Check if user is found or Not
          if (!uData || uData === null || uData.length === 0) {
            return res.status(403).json({
              error: "Unauthorized User",
            });
          } else {
            console.log("Super Admin Found".yellow);
